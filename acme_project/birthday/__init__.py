...
def calculate_birthday_countdown(birthday):
    ...
    """
    today = date.today()            # Сохраняем текущую дату в переменную today.
    this_year_birthday = get_birthday_for_year(birthday, today.year)    # Получаем день рождения в этом году с помощью вспомогательной функции (ниже).

    if this_year_birthday < today:     # Если день рождения уже прошёл, то следующий ДР будет в следующем году.
        next_birthday = get_birthday_for_year(birthday, today.year + 1)
    else:
        next_birthday = this_year_birthday              # А если в этом году ещё не было ДР, то он и будет следующим.
